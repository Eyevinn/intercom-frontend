...
export function useP2PCalls() {
  ...
  const createPeerConnection = useCallback((callId: string): RTCPeerConnection => {
    ...
    pc.oniceconnectionstatechange = () => {
      if (pc.iceConnectionState === "disconnected" || pc.iceConnectionState === "failed") {
        console.warn(`ICE connection ${pc.iceConnectionState} for call ${callId}`);
      }
    };
  ...
  const initiateCall = useCallback(async (calleeId: string) => {
    ...
    try {
      ...
      const { callId, sdpOffer, calleeName } = await API.initiateCall(calleeId);
  ...
  const endCall = useCallback(async (callId: string) => {
    ...
    dispatch({ type: "CALL_ENDED", payload: { callId, reason: "local_hangup" } });
  }, [dispatch]);

  /**
   * Toggle PTT for an outgoing call.
   */
  const togglePTT = useCallback((callId: string, talking: boolean) => {
    const pc = peerConnectionsRef.current.get(callId);
    if (!pc) return;
